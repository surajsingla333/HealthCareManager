...
export default class DataCardDoctor extends Component {
    ...
    async decryptFormHandle() {
        ...
        console.log("GET DATA of HASH", queryDoc.toString())
        console.log("GET DATA of HASH", presDoc.toString())
    ...
    render() {
        ...
        let nq = query.split(" ").join("");
        let linkQuery = false;

        if (nq === query && nq.length >= 46) {
            linkQuery = true;
        }

        let prescription = this.state.prescription;
        let np = prescription.split(" ").join("");
        let linkPrescription = false;

        if (np === prescription && np.length >= 46) {
            linkPrescription = true;
        }
                ...
                <Card>
                    <Card.Body>
                        <Card.Text>
                            ...
                            <p><span style={{ fontWeight: "bold" }}> QUERY: </span>
                                {linkQuery ?
                                    <a href={`https://gateway.ipfs.io/ipfs/${query}`} target="_blank">{query}</a> :
                                    this.state.query ? this.state.query : this.props.data
                                }
                            </p>


                            <p><span style={{ fontWeight: "bold" }}> DOCTOR: </span>{this.props.doctor}</p>
                            <p><span style={{ fontWeight: "bold" }}> PRESCRIPTION: </span>
                                {linkPrescription ?
                                    <a href={`https://gateway.ipfs.io/ipfs/${prescription}`} target="_blank">{prescription}</a> :
