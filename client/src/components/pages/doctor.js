...
class Upload extends Component {
    ...
    async getDATA() {
        ...
        var inst = this.props.state.contract;
        var currAccount = this.state.acc;
    ...
    addFile(e) {
        ...
        const patient = this.refs.patientAddInFileAdd.value.toString();
        const hash = this.refs.patientDataHashInFileAdd.value.toString();

        var fileData = this.state.files.filter(r => r.hashData.toString() === hash);
        console.log("GET FILE", fileData[0].query);
        ...
        reader.onloadend = async () => {
            ...
            console.log("RES", reader.result);
            // var uniqueHash = await genHash(reader.result);

            // console.log("FILE DATA HASH", uniqueHash);

            let ans = await ipfs.add(file);

            console.log("IPFS ANS", ans); // ans.path is the hash

            try {

                console.log("PUBLIC", this.state.publicKey);
    
                var prescriptionEncryptDoctor = await rsaEncrypt(this.state.acc.toString(), this.state.publicKey.toString(), ans.path.toString());
                ...
                if (prescriptionEncryptDoctor.result) {
                    ...
                    var prescriptionEncryptPatient = await rsaEncrypt(patient, patientPublic.toString(), ans.path.toString());
                            ...
                            <Card>
                                <Card.Body>
                                    ...
                                    <Form onSubmit={this.addFile.bind(this)}>
                                        <Form.Group controlId="addressPat">
                                            <Form.Label>Patient's Address</Form.Label>
                                            <Form.Control type="text" placeholder="Enter Patient's Address" ref='patientAddInFileAdd' />
                                        </Form.Group>

                                        <Form.Group controlId="patientFile">
                                            <Form.Label>Patient's Data Hash</Form.Label>
                                            <Form.Control type="text" placeholder="Enter File Hash" ref='patientDataHashInFileAdd' />
                                        </Form.Group>

                                        <Form.Group controlId="prescriptionFile">
                                            <a className="button browse blue">Browse     </a>
                                            <input
                                                type='file' label='Upload' ref='fileUpload'
